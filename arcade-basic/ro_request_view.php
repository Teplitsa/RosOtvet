...
?></div>
	...
	<?php if($request_region || $request_city):?>
		<?php 
			$region_filter_url = '';
			if($request_region) {
		...
		<div class="ro-request-region-block">
			<?php if($request_region):?><a href="<?=$region_filter_url?>"><?=$request_region?></a><?php endif?><?php if($request_region && $request_city):?>, <?php endif?>
			<?php if($request_city):?><a href="<?=$city_filter_url?>"><?=$request_city?></a><?php endif?>
		</div>
	<?php endif?>

	<div class="request-process-data">

	<?php if(get_field('request_status') == 'sent'):?>
		<h2><?_e('Request sent', 'arcade')?></h2>
	<?php elseif(get_field('request_status') == 'answered'):?>
				...
				if($request_text) {
					...
					<?if(@$request_official['request_send_date']):?>
						<div class="single_req_resp_date">
							<?php _e('Request Date', 'arcade')?>: <i><?=month_en2ru(date('F j, Y', strtotime(@$request_official['request_send_date'])))?></i>
							<?php
					...
					foreach($request_responses as $index1 => $request_response) {
						...
						if($request_response_text) {?>
							<div class="rosotvet-request_response_text">
								...
								<?php if(@$request_response['request_response_date']):?>
									<div class="single_req_resp_date">
										<?php _e('Response Date', 'arcade')?>: <i><?=month_en2ru(date('F j, Y', strtotime(@$request_response['request_response_date'])))?></i>
						...
						<?php $request_comment = $request_response['request_lower_comment']?>
						<?php if($request_comment):?>
	...
	<?php $request_comment = get_field('request_admin_comment')?>					
	<?php if($request_comment):?>
		<div class="rosotvet-request_lower_comment">
